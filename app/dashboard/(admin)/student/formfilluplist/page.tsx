...
export default function FormFillUpList() {
  const [registrations, setRegistrations] = useState<StudentFormFillUp[]>([]);
    ...
    const fetchRegistrations = async () => {
      try {
        const res = await fetch(
          "http://localhost:8080/api/student/formfillup"
        );
        ...
        console.error("Failed to fetch formfillups:", error);
      }
    };
    fetchRegistrations();
  }, []);
  ...
  const handleUpdateStatus = async (status: Status) => {
    ...
    try {
      const res = await fetch(
        `http://localhost:8080/api/student/formfillup/update-status`,
        {
          method: "PATCH",
